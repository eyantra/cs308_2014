...
# BBoxes must be in the format:( (topleft_x), (topleft_y) ), ( (bottomright_x), (bottomright_y) ) )
...
def get_moving_areas(boundingboxes_list, frame_size):
    numb_boxes = len(boundingboxes_list)
    area_thresh = 500
    moving_areas = []
    boundary_thresh = 4

    for i in range(numb_boxes):
        xleft = boundingboxes_list[i][0][0]
        yleft = boundingboxes_list[i][0][1]
        xright = boundingboxes_list[i][1][0]
        yright = boundingboxes_list[i][1][1]
        area = (xright-xleft)*(yright-yleft)

        right_xbound = frame_size[0]-boundary_thresh
        right_ybound = frame_size[1]-boundary_thresh
        if area <area_thresh:
            moving_areas.append(boundingboxes_list[i])

    return moving_areas

def get_centroid(moving_points):
    numb_points = len(moving_points)
    x_sum = 0
    y_sum = 0
    ...
    if numb_points == 0:
        return [1,1]
    xmean = x_sum/numb_points
    ymean = y_sum/numb_points
    return [xmean, ymean]
...
class Target:
    def __init__(self):
        if len( sys.argv ) > 1:
            ...
            is_color = True
            CAM_INDEX = 0
            self.capture = cv.CaptureFromCAM(CAM_INDEX)
            print "Now Waiting"
            c = cv.WaitKey(500)
    ...
    def detector(cap):
        ...
        while(1):
            ...
            if ret:
                ...
                lower_blue = np.array([130, 15, 215], dtype=np.uint8)
                upper_blue = np.array([173, 123, 255], dtype=np.uint8)
                ...
                if number_matches <100:
                    ...
                    res_old[Mean_x-5:Mean_x+5, Mean_y-5:Mean_y+5,:] = [0,255,0]
...
def server():
    ...
    class myHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            self.end_headers()
            self.wfile.write("%d %d %d %d" % (Mean_x, Mean_y, centroid[0], centroid[1]))
            return
...
thread.start_new_thread(detectRunner, ())
